...
def create_image_from_bytes(image_bytes: bytes, mime_type: str = "image/png") -> types.Image:
...
async def check_video_status(jobId: str = Query(..., description="Operation ID from video generation")):
    """
    ...
    try:
        ...
        print(f"Operation keys: {operation.keys() if isinstance(operation, dict) else 'N/A'}")
        
        # Check if operation is complete
        is_done = operation.get("done", False) if isinstance(operation, dict) else getattr(operation, "done", False)
        ...
        # Check for errors
        error = operation.get("error") if isinstance(operation, dict) else getattr(operation, "error", None)
        if error:
        ...
        response = operation.get("response") if isinstance(operation, dict) else getattr(operation, "response", None)
        if not response:
        ...
        if isinstance(response, dict):
            generated_videos = response.get("generatedVideos") or response.get("generated_videos") or response.get("generateVideoResponse", {}).get("generatedSamples")
        else:
            generated_videos = getattr(response, "generated_videos", None) or getattr(response, "generatedVideos", None)
        ...
        video_ref = generated_video.get("video") if isinstance(generated_video, dict) else getattr(generated_video, "video", None)
        if not video_ref:
        ...
        if isinstance(video_ref, dict) and "uri" in video_ref:
            ...
            import requests
            response = requests.get(video_uri, headers={"x-goog-api-key": os.getenv("GEMINI_API_KEY")})
            response.raise_for_status()
            video_bytes = response.content
        else:
            # Try using client.files.download
            video_file = client.files.download(file=video_ref)
            video_bytes = video_file.read() if hasattr(video_file, 'read') else video_ref.get("videoBytes", b"")
